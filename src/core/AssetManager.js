...
const context = require.context('../assets', true, /\.(jpg|png|wav|svg)$/im);

const IMG_EXTENSIONS = ['jpeg', 'jpg', 'png', 'svg'];
...
class AssetManager {
  ...
  load(assets = { images: this._images, sounds: this._sounds }, progressCallback = () => {}) {
    ...
    const loadPromises = [];
    const assetsCount = imagesCount + soundsCount
    let progressImage = 0
    let progressSound = 0

    const updateProgressImage = (val) => {
      progressImage = val
      const progress = progressImage * (imagesCount / assetsCount) +
        progressSound * (soundsCount / assetsCount)
      // console.log(progress)
      progressCallback(parseInt(progress));
    };

    const updateProgressSound = (val) => {
      progressSound = val
      const progress = progressImage * (imagesCount / assetsCount) +
        progressSound * (soundsCount / assetsCount)
      // console.log(progress)
    ...
    if (imagesCount) {
      loadPromises.push(this.loadImages(images, updateProgressImage));
    }

    if (soundsCount) {
      loadPromises.push(this.loadSounds(sounds, updateProgressSound));
    }
